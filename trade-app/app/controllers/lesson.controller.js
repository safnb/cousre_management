...
exports.create = (req, res) => {
  // Validate request
  if (!req.body.Тема || !req.body.Дата_и_время) {
    res.status(400).send({
      message: "Тема and Дата и время cannot be empty!"
    });
    return;
  }
  ...
  const lesson = {
    Тема: req.body.Тема,
    Дата_и_время: req.body.Дата_и_время,
...
exports.findAll = (req, res) => {
  const тема = req.query.тема;
  var condition = тема ? { Тема: { [Op.like]: `%${тема}%` } } : null;
